...
type EditorContext struct {
	ActiveDocID      string   `json:"activeDocID,omitempty"`
	ActiveDocTitle   string   `json:"activeDocTitle,omitempty"`
	NotebookID       string   `json:"notebookID,omitempty"`
	FocusedBlockID   string   `json:"focusedBlockID,omitempty"`
	SelectedBlockIDs []string `json:"selectedBlockIDs,omitempty"`
	VisibleBlockIDs  []string `json:"visibleBlockIDs,omitempty"`
}
...
type SessionEntry struct {
	...
	Steps         []SessionEntryStep `json:"steps,omitempty"`
	ToolCalls     []AgentToolCall    `json:"toolCalls,omitempty"`
...
func buildSystemPrompt(language string, pluginActions []PluginAction) string {
	...
	sb.WriteString("Use the skill tool to manage reusable skills: \"save\" (create/update; provide name + SKILL.md content with YAML frontmatter ---\\nname: ...\\ndescription: ...\\n--- and markdown body), \"install\" (download & install a skill from a remote source — pass url; accepts 'owner/repo' shorthand like example/ReadingList, a full GitHub URL, a raw SKILL.md URL, or a release zip URL; installed globally), \"remove\", \"rename\" (name + new_name), \"list\". When the user says \"install xxx skill\" or pastes a command like \"npx skills add owner/repo -g\", extract the owner/repo and call skill.install.")
