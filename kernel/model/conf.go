...
type AppConf struct {
	...
	Langs          []*conf.Lang         `json:"langs"`
	Lang           string               `json:"lang"`
	FileTree       *conf.FileTree       `json:"fileTree"`
	Tag            *conf.Tag            `json:"tag"`
	...
	Keymap         *conf.Keymap         `json:"keymap"`
	Sync           *conf.Sync           `json:"sync"`
	Search         *conf.Search         `json:"search"`
	Flashcard      *conf.Flashcard      `json:"flashcard"`
	AI             *conf.AI             `json:"ai"`
	Secrets        *conf.Secrets        `json:"secrets"`
	Variables      *conf.Variables      `json:"variables"`
	Extensions     *conf.Extensions     `json:"extensions"`
	Stat           *conf.Stat           `json:"stat"`
	Api            *conf.API            `json:"api"` // API
	Repo           *conf.Repo           `json:"repo"`
	NotebookCrypto *conf.NotebookCrypto `json:"notebookCrypto"`
...
func InitConf() {
	...
	if nil == Conf.FileTree.CreateDocAtTop {
...
func Close(force, setCurrentWorkspace bool, execInstallPkg int) (exitCode int, installPkgPath string) {
	...
	if !skipNewVerInstallPkg() && "" != newVerInstallPkgPath {
		if 2 == execInstallPkg || (force && 0 == execInstallPkg) {
...
func clearWorkspaceTemp(preserveInstallPkgs bool) {
	...
	os.RemoveAll(filepath.Join(util.DataDir, "%"))
	os.RemoveAll(filepath.Join(util.TempDir, "blocktree"))
