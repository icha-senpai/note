...
hljs.registerLanguage('template', function (hljs) {
    ...
    // Variable rule - starts with $, excluding dots.
    const VARIABLE_RULE = {
        className: 'variable',
    ...
    // Direct SQL statement rule - for complete SQL in double braces.
    const DIRECT_SQL_STATEMENT = {
        // Match statements beginning with SELECT.
        begin: /\b(SELECT)\b/i,
        end: /(?=\}\}|$)/,
        returnBegin: true,
        className: 'string',
    ...
    // SQL string rule - specifically for strings containing SQL statements.
    const SQL_STRING = {
        className: 'string',
        begin: /"/,
        end: /"/,
    ...
    // Detect strings after SQL query functions.
    const SQL_FUNCTION_CALL = {
        ...
        contains: [
            ...
            // SQL string following the function name.
            SQL_STRING
        ],
        relevance: 15
    };
    ...
    const ACTION_BLOCK = {
        ...
        contains: [
            COMMENT_RULE,
            SQL_FUNCTION_CALL, // SQL function call (higher priority).
            VARIABLE_RULE,
            BUILT_IN_FUNCTIONS,
            KEYWORDS_OPERATORS,
            STRING_RULE, // Normal string (lower priority).
            // Numbers.
            {
                className: 'number',
                begin: /\b\d+(\.\d+)?\b/,
    ...
    // Double-brace block rule - enhanced SQL support.
    const CURLY_BLOCK = {
        begin: /(\{\{)/,
        end: /(\}\})/,
        beginScope: 'selector-pseudo',
        ...
        contains: [
            COMMENT_RULE,
            DIRECT_SQL_STATEMENT, // Direct SQL statement (highest priority).
            VARIABLE_RULE,
            BUILT_IN_FUNCTIONS,
            KEYWORDS_OPERATORS,
            STRING_RULE,
