...
* Browser clipping extension supports context menu `Send to Scribli`
...
* Improve browser clipping extension `After clipping a web page, open document*`
* Increase the height of the editing toolbar on mobile
* Add `Show in Folder` for the database block
* Read-only mode supports previewing title images and searching by clicking on document tags
...
* Change `Search asset content` and `Add watermark to exported PDF or image` from paid to free
* Improve copy and cut
...
* Add internal kernel API `/api/ui/reloadIcon`
* Improve the `click-blockicon` event triggering after multiple blocks are selected in read-only mode
