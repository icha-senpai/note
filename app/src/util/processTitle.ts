...
export const setTitle = (title: string, showVersionTitle = false) => {
    ...
    if (showVersionTitle) {
        const versionTitle = `${workspaceName} - ${window.scribli.languages.siyuanNote} v${Constants.SCRIBLI_VERSION}`;
        ...
        document.title = `${title} - ${workspaceName} - ${window.scribli.languages.siyuanNote} v${Constants.SCRIBLI_VERSION}`;
        if (!window.scribli.config.appearance.hideToolbar && dragElement) {
