...
const genCardCount = (cardsData: ICardData, allIndex = 0) => {
    ...
    return `<span class="ariaLabel" aria-label="${window.scribli.languages.flashcardNewCard}">
    <span class="ft__error">${newIndex}</span> /
    <span class="ariaLabel ft__primary" aria-label="${window.scribli.languages.flashcardNewCard}">${cardsData.unreviewedNewCardCount}</span>
</span>
<span class="fn__space"></span>+<span class="fn__space"></span>
<span class="ariaLabel" aria-label="${window.scribli.languages.flashcardReviewCard}">
...
export const genCardHTML = (options: {
    ...
    iconsHTML = `<div class="block__icons">
        ...
        <div class="fn__space"></div>
        <div data-type="fullscreen" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show" aria-label="${window.scribli.languages.fullscreen}">
            <svg><use xlink:href="#iconFullscreen"></use></svg>
        </div>
        <div class="fn__space${options.cardsData.cards.length === 0 ? " fn__none" : ""}"></div>
        <div data-type="more" class="${options.cardsData.cards.length === 0 ? "fn__none " : ""}b3-tooltips b3-tooltips__sw block__icon block__icon--show" aria-label="${window.scribli.languages.more}">
            <svg><use xlink:href="#iconMore"></use></svg>
        </div>
        <div class="fn__space${options.isTab ? " fn__none" : ""}"></div>
        <div data-type="sticktab" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show${options.isTab ? " fn__none" : ""}" aria-label="${window.scribli.languages.openBy}">
    ...
    <div class="card__empty card__empty--space${options.cardsData.cards.length === 0 ? "" : " fn__none"}" data-type="empty">
        <div>🔮</div>
        ${window.scribli.languages.noDueCard}
    </div>
    <div class="fn__flex card__action fn__none">
        ...
        <button data-type="-1" aria-label="${window.scribli.languages.space}/${window.scribli.languages.enterKey}" class="b3-button b3-tooltips__n b3-tooltips">
            <div class="card__icon">👀</div>
            ${window.scribli.languages.cardShowAnswer}
        </button>
        <span class="fn__space"></span>
        <button data-type="-3" aria-label="0 / x" style="width: 25%" class="b3-button b3-button--cancel b3-tooltips__n b3-tooltips">
            <div class="card__icon">💤</div>
            ${window.scribli.languages.skip}${!isMobile() ? " (0)" : ""}
        </button>
    </div>
    <div class="fn__flex card__action fn__none">
        <div>
        ...
        <div>
            ...
            <button data-type="1" aria-label="1 / j / a" class="b3-button b3-button--error b3-tooltips__n b3-tooltips">
                <div class="card__icon">🙈</div>
                ${window.scribli.languages.cardRatingAgain}${!isMobile() ? " (1)" : ""}
            </button>
        </div>
        <div>
            <span></span>
            <button data-type="2" aria-label="2 / k / s" class="b3-button b3-button--warning b3-tooltips__n b3-tooltips">
                <div class="card__icon">😬</div>
                ${window.scribli.languages.cardRatingHard}${!isMobile() ? " (2)" : ""}
            </button>
        </div>
        <div>
            <span></span>
            <button data-type="3" aria-label="3 / l / d / ${window.scribli.languages.space} / ${window.scribli.languages.enterKey}" class="b3-button b3-button--info b3-tooltips__n b3-tooltips">
                <div class="card__icon">😊</div>
                ${window.scribli.languages.cardRatingGood}${!isMobile() ? " (3)" : ""}
            </button>
        </div>
        <div>
            <span></span>
            <button data-type="4" aria-label="4 / ; / f" class="b3-button b3-button--success b3-tooltips__n b3-tooltips">
                <div class="card__icon">🌈</div>
                ${window.scribli.languages.cardRatingEasy}${!isMobile() ? " (4)" : ""}
            </button>
        </div>
    </div>
</div>`;
        ...
        fetchPost("/api/filetree/getDoc", {
            ...
            onGet({
                ...
                afterCB: () => {
                    ...
                    let hasHide = false;
                    if (!window.scribli.config.flashcard.superBlock &&
                        !window.scribli.config.flashcard.heading &&
                        !window.scribli.config.flashcard.list &&
                        !window.scribli.config.flashcard.mark) {
                        hasHide = false;
                    } else {
                        if (window.scribli.config.flashcard.superBlock) {
                        ...
                        if (window.scribli.config.flashcard.heading) {
                            if (protyle.wysiwyg.element.querySelector(':scope > [data-type="NodeHeading"]')) {
                                hasHide = true;
                            }
                        }
                        if (window.scribli.config.flashcard.list) {
                            if (protyle.wysiwyg.element.querySelector(".list, .li")) {
                                hasHide = true;
                            }
                        }
                        if (window.scribli.config.flashcard.mark) {
...
export const bindCardEvent = async (options: {
    ...
    if (window.scribli.storage[Constants.LOCAL_FLASHCARD].fullscreen) {
    ...
    options.element.firstChild.addEventListener("click", (event: MouseEvent) => {
        ...
        if (typeof event.detail === "string") {
            ...
            if (fullscreenElement) {
                ...
                window.scribli.storage[Constants.LOCAL_FLASHCARD].fullscreen = !window.scribli.storage[Constants.LOCAL_FLASHCARD].fullscreen;
                setStorageVal(Constants.LOCAL_FLASHCARD, window.scribli.storage[Constants.LOCAL_FLASHCARD]);
            ...
            if (moreElement && currentCard) {
                ...
                if (filterElement.getAttribute("data-cardtype") === "all" && filterElement.getAttribute("data-id")) {
                    showMessage(window.scribli.languages.noSupportTip);
                    return;
                }
                const menu = new Menu();
                menu.addItem({
                    id: "setDueTime",
                    icon: "iconClock",
                    label: window.scribli.languages.setDueTime,
                    click() {
                        const timedialog = new Dialog({
                            title: window.scribli.languages.setDueTime,
                            content: `<div class="b3-dialog__content">
    <div class="b3-label__text">${window.scribli.languages.showCardDay}</div>
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.scribli.languages.confirm}</button>
                    ...
                    menu.addItem({
                        ...
                        click() {
                            fetchPost("/api/riff/resetRiffCards", {
                                ...
                                const minLang = window.scribli.languages._time["1m"].replace("%s", "");
                                ...
                                currentCard.nextDues = {
                                    ...
                                    4: window.scribli.languages._time["1d"].replace("%s", "").replace("1", "6")
                                };
                                actionElements[1].querySelectorAll("button.b3-button").forEach((element, btnIndex) => {
                                    if (btnIndex < 2) {
                                        return;
                ...
                menu.addItem({
                    id: "removeRiffCard",
                    icon: "iconTrashcan",
                    label: `${window.scribli.languages.remove} <b>${window.scribli.languages.riffCard}</b>`,
                    click() {
                        actionElements[0].classList.add("fn__none");
                        actionElements[1].classList.remove("fn__none");
                        if (currentCard.state === 0) {
    ...
    <div class="fn__flex-1 ft__breakword">${window.scribli.languages.forgetCount}</div>
    <div class="fn__space"></div>
    <div>${currentCard.lapses}</div>
</div><div class="fn__flex">
    <div class="fn__flex-1 ft__breakword">${window.scribli.languages.revisionCount}</div>
    <div class="fn__space"></div>
    <div>${currentCard.reps}</div>
</div><div class="fn__flex">
    <div class="fn__flex-1 ft__breakword">${window.scribli.languages.cardStatus}</div>
    <div class="fn__space"></div>
    <div class="${currentCard.state === 0 ? "ft__primary" : "ft__success"}">${currentCard.state === 0 ? window.scribli.languages.flashcardNewCard : window.scribli.languages.flashcardReviewCard}</div>
</div><div class="fn__flex${currentCard.lastReview > 0 ? "" : " fn__none"}">
    <div class="fn__flex-1 ft__breakword" style="width: 170px;">${window.scribli.languages.lastReviewTime}</div>
    <div class="fn__space"></div>
            ...
            if (filterTempElement) {
                fetchPost("/api/riff/getRiffDecks", {}, (response) => {
                    ...
                    window.scribli.menus.menu.append(new MenuItem({
                        id: "fileTree",
                        iconHTML: "",
                        label: window.scribli.languages.fileTree,
                        click() {
                            movePathTo({
                    ...
                    window.scribli.menus.menu.popup({x: filterRect.left, y: filterRect.bottom});
                });
                event.stopPropagation();
                event.preventDefault();
                return;
...
const allDone = (countElement: Element, editor: Protyle, actionElements: NodeListOf<Element>) => {
    ...
    emptyElement.innerHTML = `<div>🔮</div>${window.scribli.languages.noDueCard}`;
...
<span>${window.scribli.languages.continueReview2.replace("${count}", unreviewedCount)}</span>
<div class="fn__hr"></div>
<button data-type="newround" class="b3-button fn__size200">${window.scribli.languages.continueReview1}</button>`;
    emptyElement.classList.remove("fn__none");
    actionElements[0].classList.add("fn__none");
    actionElements[1].classList.add("fn__none");
};
