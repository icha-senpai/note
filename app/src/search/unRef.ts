...
export const openSearchUnRef = (element: HTMLElement, editor: Protyle) => {
    ...
    const localSearch = window.scribli.storage[Constants.LOCAL_SEARCHUNREF] as ISearchAssetOption;
    ...
    dragElement.addEventListener("mousedown", (event: MouseEvent) => {
        ...
        documentSelf.onmouseup = () => {
            ...
            window.scribli.storage[Constants.LOCAL_SEARCHUNREF][direction === "lr" ? "col" : "row"] = nextElement[direction === "lr" ? "clientWidth" : "clientHeight"] + "px";
            setStorageVal(Constants.LOCAL_SEARCHUNREF, window.scribli.storage[Constants.LOCAL_SEARCHUNREF]);
            if (direction === "lr") {
    ...
    dragElement.addEventListener("dblclick", () => {
        ...
        const direction = localSearch.layout === 1 ? "lr" : "tb";
        window.scribli.storage[Constants.LOCAL_SEARCHUNREF][direction === "lr" ? "col" : "row"] = "";
        setStorageVal(Constants.LOCAL_SEARCHUNREF, window.scribli.storage[Constants.LOCAL_SEARCHUNREF]);
        if (direction === "lr") {
            resize(editor.protyle);
        }
...
<span class="ft__on-surface">${window.scribli.languages.findInDoc.replace("${x}", response.data.matchedRootCount).replace("${y}", response.data.matchedBlockCount)}</span>`;
