    ...
    if (trimStartHTML.startsWith("```") || trimStartHTML.startsWith("···") || trimStartHTML.startsWith("~~~") ||
        ...
        if (trimStartHTML.indexOf("\n") === -1 && trimStartHTML.replace(/·|~/g, "`").replace(/^`{3,}/g, "").indexOf("`") > -1) {
            ...
            if (languageElement) {
                ...
                } else if (!Constants.SCRIBLI_RENDER_CODE_LANGUAGES.includes(languageElement.textContent)) {
                    window.scribli.storage[Constants.LOCAL_CODELANG] = languageElement.textContent;
                    setStorageVal(Constants.LOCAL_CODELANG, window.scribli.storage[Constants.LOCAL_CODELANG]);
                }
                if (Constants.SCRIBLI_RENDER_CODE_LANGUAGES.includes(languageElement.textContent)) {
