...
export class BlockPanel {
    ...
    constructor(options: {
        ...
        this.element.addEventListener("click", (event) => {
            if (this.element && window.scribli.blockPanels.length > 1) {
            ...
            while (target && !target.isEqualNode(this.element)) {
                if (target.classList.contains("block__icon") || target.classList.contains("block__logo")) {
                    ...
                    if (type === "close") {
                        ...
                        if (this.element.getAttribute("data-pin") === "true") {
                            target.setAttribute("aria-label", window.scribli.languages.pin);
                            target.querySelector("use").setAttribute("xlink:href", "#iconPin");
                            this.element.setAttribute("data-pin", "false");
                        } else {
                            target.setAttribute("aria-label", window.scribli.languages.unpin);
    ...
    public destroy() {
        ...
        window.scribli.blockPanels.find((item, index) => {
            if (item.id === this.id) {
                window.scribli.blockPanels.splice(index, 1);
                return true;
            }
        });
    ...
    private render() {
        ...
        if (this.refDefs.length === 1) {
            openHTML = `<span data-type="stickTab" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.scribli.languages.openInNewTab}${updateHotkeyAfterTip(window.scribli.config.keymap.editor.general.openInNewTab.custom)}"><svg><use xlink:href="#iconOpen"></use></svg></span>
<span class="fn__space"></span>`;
            /// #if !BROWSER
            openHTML += `<span data-type="open" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.scribli.languages.openByNewWindow}"><svg><use xlink:href="#iconOpenWindow"></use></svg></span>
<span class="fn__space"></span>`;
            /// #endif
        }
        let html = `<div class="block__icons block__icons--menu">
    <span class="fn__space fn__flex-1 resize__move"></span>${openHTML}
    <span data-type="pin" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.scribli.languages.pin}"><svg><use xlink:href="#iconPin"></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="close" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.scribli.languages.close}${updateHotkeyAfterTip(window.scribli.config.keymap.general.closeTab.custom)}"><svg><use xlink:href="#iconClose"></use></svg></span>
</div>
<div class="block__content">`;
        if (this.refDefs.length === 0) {
            html += `<div class="ft__smaller ft__smaller ft__secondary b3-form__space--small" contenteditable="false">${window.scribli.languages.refExpired}</div>`;
        } else {
