...
const formatSyntaxStats = (analysis: IObsidianAnalysis) => formatNonZeroStats(
    window.scribli.languages.obsidianSyntaxStats,
...
const showProgress = async (taskID: string, importing: boolean) => {
    ...
    const dialog = new Dialog({
        title: window.scribli.languages.obsidianVaultImport,
    ...
    <div style="display: flex;align-items: center;gap: 8px">
        ...
        <span data-type="status">${window.scribli.languages[importing ? "obsidianImporting" : "obsidianAnalyzing"]}</span>
    </div>
    <div class="fn__hr"></div>
    <div class="b3-progress"><div class="b3-progress__bar" style="width: 0"></div></div>
</div>
<div class="b3-dialog__action">
    <button class="b3-button b3-button--cancel" data-type="cancel">${window.scribli.languages.cancel}</button>
</div>`,
        width: "480px",
        disableClose: true,
    });
...
const showFailure = (task: IObsidianTask) => {
    if (!task.result?.incomplete) {
        showMessage(task.error || window.scribli.languages.obsidianImportFailed, 0, "error");
        return;
    }
    new Dialog({
        title: window.scribli.languages.obsidianImportFailed,
        content: `<div class="b3-dialog__content">
    <div class="ft__breakword">${escapeHtml(task.error || window.scribli.languages.obsidianImportFailed)}</div>
    <div class="fn__hr"></div>
...
const showConfirmation = (taskID: string, analysis: IObsidianAnalysis, onComplete?: () => void) => {
    ...
    const issues = [
        analysis.missingCount > 0 ? window.scribli.languages.obsidianMissingTargets.replace("${count}", analysis.missingCount.toString()) : "",
        analysis.ambiguousCount > 0 ? window.scribli.languages.obsidianAmbiguousTargets.replace("${count}", analysis.ambiguousCount.toString()) : "",
        analysis.unsupportedCount > 0 ? window.scribli.languages.obsidianUnsupportedTargets.replace("${count}", analysis.unsupportedCount.toString()) : "",
    ].filter(Boolean);
    const dialog = new Dialog({
        title: window.scribli.languages.obsidianVaultImport,
        content: `<div class="b3-dialog__content">
    <div class="b3-label">${window.scribli.languages.obsidianCompatibilityTip}</div>
    ...
    ${issues.length > 0 ? `<div class="b3-label">${window.scribli.languages.obsidianIssueSummary}<div class="b3-label__text">${issues.join("<br>")}</div></div>` : ""}
    ${warnings.length > 0 ? `<div class="b3-label">${window.scribli.languages.obsidianUnreadableFiles.replace("${count}", warnings.length.toString())}<div class="b3-label__text">${warnings.map((item) => escapeHtml(item)).join("<br>")}</div></div>` : ""}
    ${skippedCount > 0 ? `<div class="b3-label">${window.scribli.languages.obsidianSkippedSummary.replace("${count}", skippedCount.toString())}</div>` : ""}
    ${analysis.unreferencedFileCount > 0 ? `<div class="b3-label">${window.scribli.languages.obsidianUnreferencedTip.replace("${count}", analysis.unreferencedFileCount.toString())}</div>` : ""}
    <label class="b3-label" style="display: flex;align-items: center"><span style="white-space: nowrap">${window.scribli.languages.newNotebook}</span><span class="fn__space"></span><input class="b3-text-field fn__flex-1" data-type="notebook-name" value="${escapeAttr(analysis.notebookName)}"></label>
</div>
<div class="b3-dialog__action">
    <button class="b3-button b3-button--cancel" data-type="cancel">${window.scribli.languages.cancel}</button><div class="fn__space"></div>
    <button class="b3-button b3-button--text" data-type="confirm">${window.scribli.languages.import}</button>
        </div>`,
        width: "620px",
        destroyCallback: () => {
