...
export class Backlink extends Model {
    ...
    constructor(options: {
        ...
        const backlinkSort = window.scribli.config.editor.backlinkSort;
        const backmentionSort = window.scribli.config.editor.backmentionSort;
        this.element.innerHTML = `<div class="block__icons">
    <div class="block__logo fn__flex-1">${window.scribli.languages.backlinks}</div>
    ...
    <span data-type="search" class="block__icon ariaLabel" data-position="north" aria-label="${window.scribli.languages.filter}"><svg><use xlink:href='#iconFilter'></use></svg></span>
    <span class="fn__space"></span>
    ...
    <span class="fn__space"></span>
    <span data-type="sort" data-sort="${backlinkSort}" class="block__icon ariaLabel" data-position="north" aria-label="${window.scribli.languages.sort}"><svg><use xlink:href='#iconSort'></use></svg></span>
    <span class="fn__space"></span>
...
<div class="block__icons">
    <div class="block__logo fn__flex-1 fn__pointer" data-type="mention">${window.scribli.languages.mentions}</div>
    <span class="counter listMCount" style="margin-left: 0;"></span>
    <span class="fn__space"></span>
    <input class="b3-text-field search__label fn__none fn__size200" placeholder="${window.scribli.languages.filterKeywordEnter}" />
    <span data-type="search" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${window.scribli.languages.filter}"><svg><use xlink:href='#iconFilter'></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="mSort" data-sort="${backmentionSort}" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${window.scribli.languages.sort}"><svg><use xlink:href='#iconSort'></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="mExpand" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${window.scribli.languages.expand}">
        <svg><use xlink:href="#iconExpand"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="mCollapse" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${window.scribli.languages.collapse}">
        <svg><use xlink:href="#iconContract"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="layout" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${window.scribli.languages.down}">
...
<div class="backlinkMList fn__flex-1"></div>`;
        ...
        this.inputsElement.forEach((item) => {
            item.addEventListener("blur", (event: KeyboardEvent) => {
                ...
                if (inputElement.value) {
                    ...
                    filterIconElement.setAttribute("aria-label", window.scribli.languages.filter + " " + inputElement.value);
                } else {
                    filterIconElement.classList.remove("block__icon--active");
                    filterIconElement.setAttribute("aria-label", window.scribli.languages.filter);
                }
            });
        ...
        this.mTree = new Tree({
            ...
            blockExtHTML: `<span class="b3-list-item__action b3-tooltips b3-tooltips__nw" aria-label="${window.scribli.languages.more}"><svg><use xlink:href="#iconMore"></use></svg></span>`
        });
    ...
    private setLayout(element: HTMLElement) {
        if (this.mTree.element.style.flex) {
            if (this.mTree.element.style.height === "0px") {
                ...
                element.setAttribute("aria-label", window.scribli.languages.up);
                element.querySelector("use").setAttribute("xlink:href", "#iconUp");
            } else {
                this.tree.element.classList.remove("fn__none");
                this.mTree.element.removeAttribute("style");
                element.setAttribute("aria-label", window.scribli.languages.down);
                element.querySelector("use").setAttribute("xlink:href", "#iconDown");
            }
        } else {
            if (element.getAttribute("aria-label") === window.scribli.languages.down) {
                this.tree.element.classList.remove("fn__none");
    ...
    private showSortMenu(type: string, sort: string) {
        const clickEvent = (currentSort: string) => {
            ...
            if (type === "sort") {
                ...
                window.scribli.config.editor.backmentionSort = sortValue;
            }
    ...
    public saveStatus() {
        ...
        if (this.mTree.element.style.flex) {
            ...
            if (this.mTree.element.previousElementSibling.querySelector('[data-type="layout"]').getAttribute("aria-label") === window.scribli.languages.down) {
    ...
    public render(data: {
        ...
        if (!this.status[this.blockId]) {
            this.status[this.blockId] = {
                sort: window.scribli.config.editor.backlinkSort,
                mSort: window.scribli.config.editor.backmentionSort,
                scrollTop: 0,
                mScrollTop: 0,
                backlinkOpenIds: [],
                backlinkMOpenIds: [],
            ...
            if (data.mentionsCount === 0 || window.scribli.config.editor.backmentionExpandCount === -1) {
                this.status[this.blockId].backlinkMStatus = 3;
            } else {
                Array.from({length: window.scribli.config.editor.backmentionExpandCount}).forEach((item, index) => {
            ...
            if (data.linkRefsCount > 0) {
                Array.from({length: window.scribli.config.editor.backlinkExpandCount}).forEach((item, index) => {
