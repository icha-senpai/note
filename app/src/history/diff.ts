...
const genItem = (data: [], data2?: { title: string, fileID: string }[], hasUndo = true) => {
    if (!data || data.length === 0) {
        return `<li style="padding-left: 40px;" class="b3-list--empty">${window.scribli.languages.emptyContent}</li>`;
    }
    let html = "";
    ...
    <span class="b3-list-item__action ariaLabel${hasUndo ? "" : " fn__none"}" data-type="rollback" data-position="6south" aria-label="${window.scribli.languages.rollback}">
...
export const showDiff = (app: App, data: { id: string, time: string }[]) => {
    ...
    const dialog = new Dialog({
        title: window.scribli.languages.compare,
        content: "",
        ...
        while (target && target !== dialog.element) {
            if (target.classList.contains("b3-list-item") && !target.dataset.id) {
                ...
                confirmDialog("⚠️ " + window.scribli.languages.rollback,
                    window.scribli.languages.rollbackConfirm.replace("${name}", target.parentElement.textContent).replace("${time}", dayjs(parseInt(target.parentElement.dataset.created)).format("YYYY-MM-DD HH:mm:ss")),
                    () => {
...
const genHTML = (left: string, right: string, dialog: Dialog, direct: string) => {
    ...
    <span class="fn__space"></span>
    <span class="block__icon block__icon--show b3-tooltips b3-tooltips__s" aria-label="${window.scribli.languages.switchDirect}" data-direct="${direct}"><svg><use xlink:href="#iconScrollHoriz"></use></svg></span>
    <span class="fn__space"></span>
    ...
    <div class="history__side" ${isMobile() ? "" : `style="width: ${window.scribli.storage[Constants.LOCAL_HISTORY].sideDiffWidth}"`}>
        <ul class="b3-list b3-list--background">
            <li class="b3-list-item">
                <span class="b3-list-item__toggle b3-list-item__toggle--hl">
                    <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
                </span>
                <span style="padding-left: 4px" class="b3-list-item__text">${window.scribli.languages.update}</span>
        ...
        <ul class="b3-list b3-list--background">
            <li class="b3-list-item">
                ...
                <span style="padding-left: 4px" class="b3-list-item__text">${window.scribli.languages.addAttr}</span>
        ...
        <ul class="b3-list b3-list--background">
            <li class="b3-list-item">
                ...
                <span style="padding-left: 4px" class="b3-list-item__text">${window.scribli.languages.remove}</span>
