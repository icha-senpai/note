...
The audit creates a normal document, edits and saves a text block, clicks the file-tree plus button to create documents, types into a UI-created document, reopens an older UI-created document through the tree, deletes another UI-created document through the tree menu and confirmation dialog, creates and deletes an API throwaway document, creates a mixed document with a top database block, text block, HTML block, Canvas block, bottom text block, and bottom database block, then verifies the visible layout order. It also verifies Canvas toolbar visibility, card duplicate/delete, text edit/add persistence, dialog teardown, and card drag persistence.
