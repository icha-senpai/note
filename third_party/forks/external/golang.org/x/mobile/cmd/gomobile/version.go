...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/icha-senpai/note/third_party/forks/external/golang.org/x/mobile/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/icha-senpai/note/third_party/forks/external/golang.org/x/mobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/icha-senpai/note/third_party/forks/external/golang.org/x/mobile/app").CombinedOutput()
	if err != nil {
