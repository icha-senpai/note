...
func (e *singleByteEncoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	for nSrc < len(src) {
		...
		// supports it (most encodings do for the printable ASCII range)
		if r < 0x80 {
		...
		err = &UnencodableRuneError{Rune: r, Encoding: "Atari ST encoding"}
		return
	}
	return
}
...
func (e *UnencodableRuneError) Error() string {
	return "atari: cannot encode rune " + string(e.Rune) + " in " + e.Encoding
}
