...
(
	...
	go test github.com/icha-senpai/note/third_party/forks/github/dsnet/compress/$PKG_PATH -coverprofile $TMPDIR/cover.profile
	if [ $? != 0 ]; then exit 1; fi
	go tool cover -html $TMPDIR/cover.profile -o cover.html

	# Perform performance profiling.
	if [ $# != 0 ]; then
		go test -c github.com/icha-senpai/note/third_party/forks/github/dsnet/compress/$PKG_PATH
		if [ $? != 0 ]; then exit 1; fi
