...
for pkg in $(go list -f '{{if not .Goroot}}{{.ImportPath}}{{end}}' $(go list -f '{{.ImportPath}} {{join .Deps " "}}' github.com/icha-senpai/note/third_party/forks/github/gopherjs/gopherjs)); do
    copyGoPackage "$pkg" "$tmp/src/example.org/hello/vendor/$pkg"
done
# Two special cases
for pkg in "github.com/icha-senpai/note/third_party/forks/github/gopherjs/gopherjs/js" "github.com/icha-senpai/note/third_party/forks/github/gopherjs/gopherjs/nosync"; do
    copyGoPackage "$pkg" "$tmp/src/example.org/hello/vendor/$pkg"
done

cp -r "$(go list -f '{{.Dir}}' 'github.com/icha-senpai/note/third_party/forks/github/gopherjs/gopherjs/compiler/natives')/src" \
      "$tmp/src/example.org/hello/vendor/github.com/icha-senpai/note/third_party/forks/github/gopherjs/gopherjs/compiler/natives/src"
...
go install example.org/hello/vendor/github.com/icha-senpai/note/third_party/forks/github/gopherjs/gopherjs
