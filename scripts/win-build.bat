...
if "%TARGET%"=="amd64" (
    ...
    REM The appx stage depends on electron-builder's win-unpacked output, so build amd64 Electron too
    set BUILD_AMD64=1
    set BUILD_APPX_AMD64=1
) else if "%TARGET%"=="appx-arm64" (
    REM The appx stage depends on electron-builder's win-arm64-unpacked output, so build arm64 Electron too
    set BUILD_ARM64=1
...
if defined BUILD_AMD64 (
    ...
    set GOARCH=amd64
    go build -tags "fts5 sqlcipher" -o "%PROJECT_ROOT%\app\kernel\Scribli-Kernel.exe" -ldflags "-s -w" .
    if errorlevel 1 (
        exit /b %errorlevel%
    )
)
if defined BUILD_ARM64 (
    ...
    go build -tags "fts5 sqlcipher" -o "%PROJECT_ROOT%\app\kernel-arm64\Scribli-Kernel.exe" -ldflags "-s -w" .
    if errorlevel 1 (
        exit /b %errorlevel%
    )
)
...
if defined BUILD_APPX_AMD64 (
    ...
    call electron-windows-store --input-directory "%PROJECT_ROOT%\app\build\win-unpacked" --output-directory "%PROJECT_ROOT%\app\build" --package-version 1.0.0.0 --package-name Scribli --manifest "%PROJECT_ROOT%\app\appx\AppxManifest.xml" --assets "%PROJECT_ROOT%\app\appx\assets" --make-pri true
...
if defined BUILD_APPX_ARM64 (
    ...
    call electron-windows-store --input-directory "%PROJECT_ROOT%\app\build\win-arm64-unpacked" --output-directory "%PROJECT_ROOT%\app\build" --package-version 1.0.0.0 --package-name Scribli-arm64 --manifest "%PROJECT_ROOT%\app\appx\AppxManifest-arm64.xml" --assets "%PROJECT_ROOT%\app\appx\assets" --make-pri true

    rmdir /S /Q "%PROJECT_ROOT%\app\build\pre-appx" 1>nul
)
:skipappx

REM Create the hard link after packaging so scribli.exe is not included in the installer
if defined BUILD_AMD64 (
    mklink /H "%PROJECT_ROOT%\app\kernel\scribli.exe" "%PROJECT_ROOT%\app\kernel\Scribli-Kernel.exe"
)
if defined BUILD_ARM64 (
    mklink /H "%PROJECT_ROOT%\app\kernel-arm64\scribli.exe" "%PROJECT_ROOT%\app\kernel-arm64\Scribli-Kernel.exe"
)
